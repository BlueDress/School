...
namespace Collector
{
    public class Spy
    {
        ...
        public string CollectGettersAndSetters(string className)
        {
            ...
            var type = typeof(Hacker);
            var allMethods = type.GetMethods(BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public | BindingFlags.Static);

            foreach (var method in allMethods.Where(met => met.Name.StartsWith("get")))
            {
                sb.AppendLine($"{method.Name} will return {method.ReturnType}");
            }

            foreach (var method in allMethods.Where(met => met.Name.StartsWith("set")))
            {
