...
namespace ImplementQuadTree
{
    public class QuadTree<T> where T : IBoundable
    {
        ...
        {
            var collusionCandidates = new List<T>();

            GetCollusionCandidates(this.root, bounds, collusionCandidates);

            return collusionCandidates;
        }
        ...
        private static int GetQuadrant(Node<T> node, Rectangle bounds)
        {
            ...
            var inLeftQuadrant = verticalMidpoint <= bounds.X1 && bounds.X2 <= node.Bounds.X2;
            var inRightQuadrant = node.Bounds.X1 <= bounds.X1 && bounds.X2 <= verticalMidpoint;
        ...
        private void GetCollusionCandidates(Node<T> node, Rectangle bounds, List<T> results)
        {
            ...
            else
            {
                if (node.Children != null)
                {
                    GetCollusionCandidates(node.Children[quadrant], bounds, results);
                }
